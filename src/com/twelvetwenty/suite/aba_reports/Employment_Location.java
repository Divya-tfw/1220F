...
public class Employment_Location extends App_Specific_Keywords {
	...
	/*****************************************************************************************************************
	 * Author : Divya Raju.R LastModifiedDate : 10-6-2015 Annotation : @Test
	 * MethodName : test_Employment_Location Description : This method is used
	...
	public void test_Employment_Location(Hashtable<String, String> data) {
		...
		GlobalVariables.testCaseIdentifier = data
				.get(TestBaseConstants.AUTOMATION_ID);

		Logs.openLogFile(GlobalVariables.logFolderPath,
				GlobalVariables.testCaseIdentifier);
		GlobalVariables.dataRunStatus = data
				.get(TestBaseConstants.DATA_RUNMODE);
		...
		try {
			...
			rATUConfigInfo(TestBaseConstants.ATU_INDEX_PAGE_DESCRIPTION,
					...
					GlobalVariables.scriptName, TestBaseConstants.AUTHOR_NAME,
			...
			launchSite(cleanPath(GlobalVariables.CONFIG
					.getProperty(TestBaseConstants.SITE + url)));
			...
			loginToSite(data.get(TestBaseConstants.WAIT_VALUE),
					GlobalVariables.CONFIG
							.getProperty(TestBaseConstants.SITE_USER_NAME),
					cleanContent(GlobalVariables.CONFIG
							.getProperty(TestBaseConstants.SITE_PASSWORD)));
			webdriverWait(data.get("iWait"));

			// click std reports
			click(TestBaseConstants.BUTTON_STANDARD_REPORTS,
			...
			// click on report
			rATUStatus(TestBaseConstants.RESULT_PASSVALUE,
					"Click on " + data.get(TestBaseConstants.SUB_REPORT_NAME)
							+ " report");
			...
			GlobalVariables.driver.findElement(
					By.xpath("//tr[td[contains(text(),'"
							+ data.get(TestBaseConstants.SUB_REPORT_NAME)
							+ "')]]//*[text()='Generate']")).click();
			Logs.infoLog("Click on "
					+ data.get(TestBaseConstants.SUB_REPORT_NAME) + " report");
			...
			if (data.get(TestBaseConstants.CUT_OFF_DATE).equalsIgnoreCase(
					TestBaseConstants.CUT_OFF_DATE_WITHOUT)) {
				/*
				 * GlobalVariables.driver.findElement(By.id("LawCutoffDate")).click
				 * ();
				 * GlobalVariables.driver.findElement(By.id("LawCutoffDate"))
				 * .clear();
				 */

				getObjectValue(TestBaseConstants.DROP_DOWN_CUT_OFF_DATE)
						.click();
				getObjectValue(TestBaseConstants.DROP_DOWN_CUT_OFF_DATE)
						.clear();
			...
			click(TestBaseConstants.BUTTON_GENERATE_REPORT,
...

			{
				...
				, GlobalVariables.testCaseIdentifier, "Writing contents of "
						...
						+ " to excel",
						GlobalVariables.OR
								.getProperty("report_Employment_Location"),
						data.get(TestBaseConstants.SUB_REPORT_NAME));
