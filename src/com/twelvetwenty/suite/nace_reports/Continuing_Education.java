...
public class Continuing_Education extends App_Specific_Keywords {

	/*****************************************************************************************************************
	 * Author : Divya Raju.R LastModifiedDate : 19-1-2015 Annotation : @Test
	 * MethodName : test_Continuing_Education Description : This method is used
	 * to perform required functionality test on app
	 * 
	 ***************************************************************************************************************/
	@Test(dataProvider = "ExcelData")
	public void test_Continuing_Education(Hashtable<String, String> data) {
		// Start of script
		...
		Logs.openLogFile(GlobalVariables.logFolderPath,
				GlobalVariables.testCaseIdentifier);
		GlobalVariables.dataRunStatus = data.get("Runmode");
		GlobalVariables.suiteRunStatus = ExcelTestUtil
				.suiteRunstatus(GlobalVariables.suiteName);
		GlobalVariables.testRunStatus = ExcelTestUtil.testRunStatus(
				GlobalVariables.suiteName, GlobalVariables.scriptName);
		boolean trs = ExcelTestUtil.checkSkiptests(
				GlobalVariables.suiteRunStatus, GlobalVariables.testRunStatus,
				GlobalVariables.dataRunStatus);

		dbUpdate(trs, GlobalVariables.scriptName,
				GlobalVariables.testCaseIdentifier);
		Logs.infoLog("--------Execution of test---- "
				+ GlobalVariables.scriptName + "  Started----");
		...
		try {
			...
			data.get("sSub_Report_Name"), GlobalVariables.scriptName, "Divya",
					"1.0");
			...
			launchSite(cleanPath(GlobalVariables.CONFIG.getProperty("site_"
					+ url)));
			...
			// click on report
			rATUStatus("Pass", "Click on " + data.get("sSub_Report_Name")
					+ " report");
			...
			selectValueFromDropDown(getObjectValue("dpdown_GraduationYr"),
					"Text", data.get("iGraduationYr"),
					"Select Graduation Year-->" + data.get("iGraduationYr"));
			Logs.infoLog("Select Graduation Year-->"
					+ data.get("iGraduationYr"));
			Thread.sleep(1000);

			// select graduation term
			selectValueFromDropDown(getObjectValue("dpdown_GraduationTerm"),
			...
			if (data.get("sCollege").contains("FC AT ROSE HILL")) {
				GlobalVariables.driver
						.findElement(
								By.xpath("//*[@id='reportsForm']/div/ul/li[3]/div/button"))
						.click();
				Thread.sleep(2000);
				GlobalVariables.driver
						.findElement(
								By.xpath("//*[@id='reportsForm']/div/ul/li[3]/div/ul/li[3]/a/label/input"))
						.click();
			}
			Thread.sleep(3000);
			// select reporting major
			selectValueFromDropDown(getObjectValue("dpdown_Reporting_Major"),
					"Index", data.get("sReportingMajor"),
					"Select Reporting major-->" + data.get("sReportingMajor"));
			Thread.sleep(2000);
			// select degree level
			selectValueFromDropDown(getObjectValue("dpdown_DegreeLevel"),
					"Text", data.get("sDegreeLevel"), "Select Degree level-->"
							+ data.get("sDegreeLevel"));
			Thread.sleep(2000);

			if (data.get("sSchoolName")
					.equalsIgnoreCase("Columbia_Centralized")) {
				// select department
				// //*[@id='reportsForm']/div/ul/li[5]/div/ul/li[1]/div/input
				GlobalVariables.driver
						.findElement(
								By.xpath("//*[@id='reportsForm']/div/ul/li[5]/div/button"))
						.click();
				// type to search value
				Thread.sleep(2000);
				GlobalVariables.driver
						.findElement(
								By.xpath("//*[@id='reportsForm']/div/ul/li[5]/div/ul/li[1]/div/input"))
						.click();
			...
			// perform write or read to excel using Build type value present in
			// excel
...

			{
				...
				mT1_TH1_TCN_WriteXLSX(GlobalVariables.testCaseIdentifier

				, GlobalVariables.testCaseIdentifier, "Writing contents of "
						+ data.get("sSub_Report_Name") + " to excel",
						GlobalVariables.OR
								.getProperty("report_Continuing_Education"),
						data.get("sSub_Report_Name"));

			} else if (GlobalVariables.CONFIG.getProperty(
					TestBaseConstants.BUILD_TYPE).equalsIgnoreCase(
...

			{
				...
				mT1_TH1_TCN_ReadXLSX(
						...
						"Validating contents of table "
								+ data.get("sSub_Report_Name") + "- with excel");
			}
